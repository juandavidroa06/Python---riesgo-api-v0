...
| H10 | El `README.md` documenta el arranque con `--reload` como apto para producción | La sección de puesta en marcha usa `uvicorn main:app --reload` y afirma textualmente que ese mismo comando sirve para producción | M5 · 4. Uvicorn y el arranque en producción | `v0-semilla` | `grep -i reload README.md` | `uvicorn main:app --reload --host 0.0.0.0 --port 8000` — `El mismo comando sirve en el servidor de producción. --reload es cómodo` | Se documentó `uvicorn main:app --host 0.0.0.0 --port 8000 --workers 2`, sin `--reload`, y se corrigió el texto que lo llamaba apto para producción |
| H11 | El decorador `con_registro` atrapa cualquier excepción —incluidos errores de programación, no solo de datos— y devuelve `None` en silencio en vez de propagarla | El `try/except Exception` interno intercepta hasta un `TypeError` por argumentos faltantes, lo registra en el log con un mensaje genérico y retorna `None` como si la llamada hubiera tenido éxito | M1 · 6. Decoradores como guardianes | `v0-semilla` | `python -c "from dominio import EvaluadorRiesgo as E; ev = E('POL-1'); print(ev.puntuar({}))"` | `[registro] puntuar falló: EvaluadorRiesgo.puntuar() missing 1 required positional argument: 'payload'` — `None` | Se quitó el `try/except`; el decorador ahora solo registra en el log (`logger.info`) y deja propagar cualquier excepción |
